<div class="container-fluid">
	<div class="row mt-4">
		<div class="col">
			<h3>
				Panel de administrador
			</h3>
            <a href="admin/listCards" class="btn btn-primary btn-block" type="button">Listar cartas</a>
            <a href="admin/addCard" class="btn btn-primary btn-block" type="button">Agregar carta</a>
			<hr>
			<a href="admin/listExpansions" class="btn btn-secondary btn-block" type="button">Listar expansiones</a>
            <a href="admin/addExpansion" class="btn btn-secondary btn-block" type="button">Agregar expansión</a>
		</div>
	</div>
</div>